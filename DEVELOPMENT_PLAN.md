...
- [x] Restore baseline sans-serif readability in shared face-label rendering by bridging `UIFont` directly into CoreText glyph extraction (avoid name-based serif fallback across platforms).
- [x] Remove duplicated single-die polyhedral geometry/orientation code by making `DiceSingleDieSceneGeometryFactory` the canonical iOS/watch SceneKit source and covering every factory function with automated tests.
...
- [x] Set watch default background to `black` while preserving existing iOS/iPadOS/macOS defaults.
- [x] Add regression tests validating `black` mode color output is exact zero RGB and unaffected by theme switching.
- [x] Ensure watch background switching reuses the same shader/material pipeline used on iOS where feasible (no watch-only parallel renderer path for equivalent behavior).
...
- [x] Fix watch main die interaction/background regressions after full-screen refactor: use gesture-recognizer-driven die surface for tap-roll + long-press customize, guarantee watch extension owns required table resources (`DiceTableSurfaceShader.metal`, `stripes` asset), and add regression tests for lifecycle/scene fallback recovery.
...
- [x] Define tvOS focus appearance for dice, cards, and actions (scale, parallax/shadow, glow, sound) with contrast/readability thresholds suitable for sofa distance.
